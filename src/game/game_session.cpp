...
GameSession::GameSession(SDL_Window* win, SDL_Renderer* rend, void* game_state, int w, int h)
    : window(win), renderer(rend), window_w(w), window_h(h), gameConf(1920, 1080, 1920.f/1080.f), 
    ui_manager(&gameConf, win, rend, &engine, game_state), game_event(&ui_manager, renderer), game_state(game_state),
    engine(game_state)
{
