...
void GameConfigerScene::handleEvents(const SDL_Event& event) {
    ...
    if (ui_manager->game_state->game_mode == "B V B") {
        if (btn_side_baagh.handleEvent(event)) return;
        if (btn_side_goat.handleEvent(event)) return;
        if (btn_diff_low.handleEvent(event)) return;
        if (btn_diff_med.handleEvent(event)) return;
        if (btn_diff_high.handleEvent(event)) return;
    }
